...
// wksaToken
contract wksaToken is ERC20("wKSA", "wKSA"), Ownable {
...
contract Caesar is Ownable {
    ...
    constructor(
        ...
        for (uint256 i = 2; i <= 6; i++) {
            levelBonus[i] = levelBonus[i-1].div(2);
        }
        lastUser = 1;
    ...
    function getWeight(uint256 _nksn, uint256 _nkcn, uint256 _nkcm, uint256 _inviteAmount) public view returns (uint256) {
        ...
        if(_nksn >= 6){
            weight = weight.add(levelBonus[6]).mul(15);
        }
        else{
            weight = weight.add(levelBonus[_nksn]).mul(15);
        }
        if(_nkcn >= 6){
            weight = weight.add(levelBonus[6]).mul(10);
        }
        else{
            weight = weight.add(levelBonus[_nkcn]).mul(10);
        }
        if(_nkcm >= 6){
            weight = weight.add(levelBonus[6]).mul(5);
        }
        else{
    ...
    function updatePoolWeight(uint256 _pid, address _user) public {
        ...
        uint newWeight = getWeight(user.nksnList.length, user.nkcnList.length, user.nkcmList.length, inviterList[_user].length);
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _inviter) public {
        ...
        if(isInviter){
            UserInfo storage inviterInfo = userInfo[_pid][idToAddr[_inviter]];
            if(inviterInfo.amount > 0){
                uint256 pending = inviterInfo.amount.mul(inviterInfo.weight).div(1e12).mul(pool.accWKSAPerShare).div(1e12).sub(inviterInfo.rewardDebt);
                safeWKSATransfer(msg.sender, pending);
            }
            updateUserWeight(_pid, idToAddr[_inviter]);
            inviterInfo.rewardDebt = inviterInfo.amount.mul(inviterInfo.weight).div(1e12).mul(pool.accWKSAPerShare).div(1e12);
        }
    ...
    function withdrawNKSN(uint256 nksnId, uint256 _pid) public {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        for(uint256 i = 0; i < user.nksnList.length; i ++){
            if(user.nksnList[i] == nksnId){
                user.nksnList[i] = user.nksnList[user.nksnList.length-1];
                user.nksnList.pop();
    ...
    function withdrawNKCN(uint256 nkcnId, uint256 _pid) public {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        for(uint256 i = 0; i < user.nkcnList.length; i ++){
            if(user.nkcnList[i] == nkcnId){
                user.nkcnList[i] = user.nkcnList[user.nkcnList.length-1];
                user.nkcnList.pop();
    ...
    function withdrawNKCM(uint256 nkcmId, uint256 _pid) public {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        for(uint256 i = 0; i < user.nkcmList.length; i ++){
            if(user.nkcmList[i] == nkcmId){
                user.nkcmList[i] = user.nkcmList[user.nkcmList.length-1];
                user.nkcmList.pop();
